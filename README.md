...
`dashboard-package.yaml` does **not** need to be copied locally when using the v1.1 example: it is loaded remotely from this GitHub repository.
...
If your robot uses different topics, change only these values.

### 3. Configure ESPHome secrets

The repository does **not** contain real Wi-Fi credentials or API keys.
...
These packages still need to know the source entities from your Home Assistant installation. Before using them, read:

**`home-assistant/CONFIGURATION.md`**

It lists every source entity that must be replaced. The generated `sensor.display_*` entity IDs are intentionally kept stable because the ESP32 dashboard reads those helper names directly.

### 5. Allow Home Assistant actions
...
This permission is required by dashboard controls that call Home Assistant actions, including MQTT commands used by the robot controls.
...
`waveshare-dashboard.yaml` is retained as the legacy/reference monolithic configuration from the original v1.0 project and is not the recommended starting point for a new v1.1 installation.
